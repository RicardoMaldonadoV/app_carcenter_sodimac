-- 5. Consulta de Clientes que han comprado un acumulado $100.000 en los ultimos 60 dias
SELECT C.P_NOMBRE||' '||C.s_NOMBRE||' '||C.P_APELLIDO||' '||C.S_APELLIDO AS NOMBRE_CLIENTE,
 ...
 WHERE F.FECHA_FACTURA >= (SELECT SYSDATE -60 FROM DUAL) -- En los ultimos 60 dias 
...
-- 6. Consulta de los 100 productos mas vendidos en los ultimos 30 dias
SELECT *
  FROM (
    SELECT 
      R.DESCRIPCION AS "TOP_100_NOMBRE_PRODUCTO", 
      R.ID_PRODUCTO,
...
--7. Consulta de las tiendas que han vendido mas de 100 UND del producto 100 en los ultimos 60 dias.
...
--8. Consulta de todos los clientes que han tenido mas de un mantenimento en los ultimos 30 dias.
SELECT C.P_NOMBRE||' '||C.s_NOMBRE||' '||C.P_APELLIDO||' '||C.S_APELLIDO AS NOMBRE_CLIENTE,
       C.DOCUMENTO,
 ...
 WHERE F.FECHA_FACTURA >= (SELECT SYSDATE -30 FROM DUAL) -- En los ultimos 30 dias 
 GROUP BY (C.P_NOMBRE,C.S_NOMBRE,C.P_APELLIDO,C.S_APELLIDO,C.DOCUMENTO)
...
--ALTER TRIGGER TRIGGER_INVENTARIO_ACTUALIZAR_STOCK ENABLE;